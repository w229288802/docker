...
println_and_run "docker run -it --rm --name=${APP_NAME} --hostname=${APP_NAME}-master -e PATH=${PATH}:${HIVE_HOME}/bin --network hadoop -v ${HIVE_HOME_VOLUME}:/opt ${APP_NAME}:v${HIVE_VERSION}"
