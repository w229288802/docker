...
println_and_run "docker rm -f yum-${APP_NAME} >/dev/null && docker run -d --name=yum-${APP_NAME} --hostname=yum-${APP_NAME} -p 3636:80 -v ${CDH_PATH}:/usr/share/nginx/html/cdh6.3.2 -v ${CDH_PATH}/cm6.3.1:/usr/share/nginx/html/cm6.3.1 nginx"

println_and_run "docker build  --build-arg HOST_IP=${HOST_IP}:3636 -t ${APP_NAME}:v${CDH_VERSION} ${FILE_PATH}/build-support"

println_and_run "docker rm -f ${APP_NAME} 2>/dev/null && docker run --rm -d --name=${APP_NAME} --hostname=${APP_NAME} --network=hadoop --privileged=true ${APP_NAME}:v${CDH_VERSION} /sbin/init"
