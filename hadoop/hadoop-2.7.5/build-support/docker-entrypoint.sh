...
echo -n "master " > ~/.ssh/known_hosts &&  sed -r "s|root@\w+||g" /etc/ssh/ssh_host_rsa_key.pub >>  ~/.ssh/known_hosts
echo -n "slave1 " >> ~/.ssh/known_hosts &&  sed -r "s|root@\w+||g" /etc/ssh/ssh_host_rsa_key.pub >> ~/.ssh/known_hosts
...
println "check activated master"

while nc -w 1 master 22 </dev/null >/dev/null 2>&1;[[ $? == 0 || $? == 2 ]] #等待master停止
do
  sleep 0.2s
done
println "check inactivated master"
fi
tail -f /dev/null